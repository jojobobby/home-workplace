...
public static class GoalEndpoints
{
    ...
    {
        app.MapPost("/goals", async (CreateGoalRequest req, GoalBook goals, EmployeeCatalog cat, RunSupervisor sup, IContextApiClient rooms, CancellationToken ct) =>
        {
            ...
            var manager = cat.Find(req.Manager)!;
            if (cat.GetState(manager.Id).Status == EmployeeStatus.Asleep)
                await rooms.PostAsync(goal.Room, "foreman", "Foreman", null,
                    $"{manager.Name} ({manager.Id}) is asleep; the goal starts when they wake at {manager.Schedule.WakeTime:HH\\:mm}, or wake them now.", ct);
