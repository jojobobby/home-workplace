...
public sealed class FakeForemanApi : IForemanApi
{
    ...
    public Task<TaskDto> CreateTicketAsync(CreateTicketRequest r, CancellationToken ct = default) { var t = Task("ticket" + Tasks.Count, assignee: "") with { Title = r.Title, Role = r.Role }; Tasks[t.Id] = t; return Rec($"createTicket:{r.Title}:{r.Role}{(r.BudgetUsd is { } b ? ":" + b : "")}", t); }
